...
export function SpendChart({
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-card/95 via-card/90 to-primary/10 p-6 md:p-8 space-y-6 shadow-[0_18px_40px_-28px_rgba(0,0,0,0.7)]">
      ...
      <div className="relative flex flex-col sm:flex-row sm:items-center justify-between border-b border-border/70 pb-4 gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <span className="inline-flex items-center rounded-full border border-border/70 bg-background/70 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] text-muted-foreground">
            ${totalSavings.toLocaleString()} potential savings
          </span>
          <div className="flex items-center gap-4 text-[11px] font-semibold uppercase tracking-[0.2em] text-muted-foreground">
            <span className="flex items-center gap-2">
              <span className="h-2.5 w-2.5 rounded-sm bg-muted-foreground/50" />
              Current
            </span>
            <span className="flex items-center gap-2">
              <span className="h-2.5 w-2.5 rounded-sm bg-primary shadow-[0_0_10px_rgba(255,255,255,0.15)]" />
              Optimized
            </span>
          </div>
        </div>
      ...
      <div className="relative space-y-6">
        {recommendations.map((rec, idx) => {
          ...
          return (
            <div
              key={idx}
              className="space-y-3 rounded-xl border border-border/60 bg-background/60 p-4 shadow-sm"
            >
